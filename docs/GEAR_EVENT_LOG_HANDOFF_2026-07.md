...
4. **Item timelines in UI:** `item.html` (generated by `generate_item_timeline`) loads gear shards client-side; resolve by `?i=` / `?n=` (case-insensitive name) or search box; shows current holders + ownership log. Python helpers: `item_history()`, `filter_events_for_item`, `reconstruct_holders_for_item`, `detect_oscillations()`.
