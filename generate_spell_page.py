...
""" + _gear_event_fetch_client_js() + """
        ...
        function findKeyCI(obj, name) {
            ...
            for (const k of Object.keys(obj)) {
                if (String(k).toLowerCase() === needle) return k;
            }
            return null;
        }

        function resolveCharName(query, baseline, stateMap) {
            const sources = [
                (baseline && baseline.characters) || null,
                (baseline && baseline.inventories) || null,
                stateMap || null,
                CHAR_GUILD_MAP || null,
            ];
            for (const obj of sources) {
                const hit = findKeyCI(obj, query);
                if (hit) return hit;
            }
            return query;
        }
        ...
        function filterEventsForChar(events, charName) {
            return (events || []).filter(ev => namesEqual(ev.c, charName));
        }
        ...
        function buildItemNameMap(baseline, charName) {
            ...
            const invKey = findKeyCI((baseline.inventories || {}), charName) || charName;
            for (const item of ((baseline.inventories || {})[invKey] || [])) {
        ...
        function buildAaTimeline(baseline, charEvents, charName) {
            const charKey = findKeyCI((baseline.characters || {}), charName) || charName;
            const bl = ((baseline.characters || {})[charKey]) || {};
            let running = (bl.aa_unspent || 0) + (bl.aa_spent || 0);
            const baselineDate = baseline.baseline_date || '';
            const rows = [];
            if ((baseline.characters || {})[charKey] || running > 0) {
        ...
        function buildCurrentHoldings(baseline, gearEvents, charName) {
            const invKey = findKeyCI((baseline.inventories || {}), charName) || charName;
            const baselineItems = ((baseline.inventories || {})[invKey] || []);
        ...
        async function render() {
            ...
            try {
                ...
                const charName = resolveCharName(CHAR_NAME, baseline, stateMap);
                document.getElementById('page-title').textContent = charName + ' — Timeline';
                const charEvents = filterEventsForChar(eraChars, charName);
                const charGear = filterEventsForChar(gear, charName);
                const state = stateMap[charName] || {};
                const guildKey = findKeyCI(CHAR_GUILD_MAP, charName) || charName;
                const guild = state.guild || CHAR_GUILD_MAP[guildKey] || '';
                const guildPart = guild ? ' &lt;' + esc(guild) + '&gt;' : '';
                const mageloSlug = charName.toLowerCase().replace(/ /g, '_');
                meta.innerHTML = '<strong>' + esc(charName) + guildPart + '</strong> · '
                ...
                const nameMap = buildItemNameMap(baseline, charName);
                const aaRows = buildAaTimeline(baseline, charEvents, charName);
                const holdings = buildCurrentHoldings(baseline, charGear, charName);
                const sinceBaseline = baselineOnlyItems(holdings, charGear, charName);
                const gearLog = buildGearEventLog(charGear, charName, nameMap);
                const trackedRows = buildTrackedGearEventLog(charGear, charName, nameMap, baseline);
                const sinceBaselineTracked = baselineOnlyTracked(holdings, charGear, charName);
