...
The generated HTML is deployed to the `gh-pages` branch and will be available at:
`https://YOUR_USERNAME.github.io/YOUR_REPO/` (as `index.html`)

**Note**: The HTML file is not in the `main` branch - it's generated by the workflow and deployed to `gh-pages`. You can view it:
- On GitHub Pages: `https://ammordius.github.io/NAGD-spell-inventory/`
- In the repository: Switch to the `gh-pages` branch to see the deployed files
