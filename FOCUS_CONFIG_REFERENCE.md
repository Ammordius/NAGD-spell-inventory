...
## Focus Affliction Haste (DoT/debuff-only, separate from Detrimental Spell Haste)

**Affliction Haste** is a separate spell-haste type: it only affects **DoTs and debuffs** (33% cap in-game). It does **not** affect nukes, so it is **useless for Wizard and Magician** (weight 0). It is useful for Necromancer, Shaman, Enchanter, Druid, Beastlord and is weighted there.
...
- **Spell Mana Efficiency:** `SPELL_MANA_EFFICIENCY_CATEGORY_MAP` ~line 547 (Det, Bene, Nuke, LDD, Sanguine, All). **Conservation of Solusek** = `'Det'` (general detrimental: nukes + debuffs; e.g. Talisman of Tainted Energy). **Conservation of Bertoxxulous** = `'LDD'` (long-duration debuff only). **Conservation of Xegony** = `'All'`. **Preservation of the Akheva** = `'All'` (18% all; Mask of Secrets, Mask of Judgement). When sub is `'All'`, focus sources are added to Bene, Det, Nuke, and LDD (like Spell Haste All → both Bene and Det). Display uses effective LDD = max(LDD, Det, All) so Det gear shows for LDD when the class has no LDD-specific focus. Sanguine = self-only; weight 0 for most classes.
- **item_stats percentage:** When merging focus from `data/item_stats.json`, if an entry has `focusPct` or `focusPercentage` (numeric), that value is used; otherwise the default from `_infer_focus_category` (e.g. 15%) is used. Use `focusPct` for items like Mask of Secrets (18% beneficial) when the tier differs from the default.
